...
cmd_session_backup() {
  local sid="${1:-}"
  [[ -n "$sid" ]] || { log_error "usage: ocm session backup <session_id>"; return 1; }

  load_config
  [[ "$sid" =~ ^[a-zA-Z0-9_-]+$ ]] || { log_error "bad session id: $sid"; return 1; }

  local backup_dir="${OCM_SESSION_BACKUP_DIR:-$HOME/.local/share/opencode/session-backups}"
  ...
  local sql_sid
  sql_sid=$(sql_escape "$sid") || { log_error "invalid session id for SQL: $sid"; return 1; }

  sqlite3 -readonly "$OCM_OPencode_DB" >"$out" <<EOF
.mode insert session
...
cmd_session_restore() {
  local file="${1:-}"
  [[ -n "$file" ]] || { log_error "usage: ocm session restore <file.sql>"; return 1; }
...
cmd_session_cleanup() {
  ...
  log_info "Cleaning probe sessions (ocmm-probe)..."
  ...
  while IFS=$'\t' read -r sid title; do
    ...
    if sqlite3 -readonly "$OCM_OPencode_DB" \
      ...
      delete_session "$sid" && { deleted=$((deleted+1)); log_info "deleted probe session $sid"; }
    fi
...
cmd_session() {
  ...
  case "$subcmd" in
    ...
    *)
      ...
      echo "Usage: ocm session [list|backup|restore|cleanup]"
      return 1
      ;;
  esac
}